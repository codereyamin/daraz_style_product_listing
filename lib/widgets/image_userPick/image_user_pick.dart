...
import 'package:daraz_style_product_listing/main_app_entry.dart';
...
import 'package:daraz_style_product_listing/utils/app_snack_bar.dart';
...
Future<void> appUserImagePic({required ImageSource source, required Function(String) callBack}) async {
  try {
    ...
    if (source == ImageSource.camera) {
      ...
      if (status.isDenied) {
        var response = await askFirst(title: "Camera Access Required", content: """We need access to your camera so you can update your profile picture""", acceptButton: "Continue");
        if (response) {
      ...
      if (status.isPermanentlyDenied) {
        var response = await getCallAgainPermission(
          ...
          content: """We need access to your camera so you can take or update your profile picture. We respect your privacy and only use the camera when you choose to use this feature.
