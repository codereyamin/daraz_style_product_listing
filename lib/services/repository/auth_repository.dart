...
class AuthRepository {
  ...
  Future<bool> login({required String email, required String password, required String fcmToken, required String deviceId}) async {
    try {
      Map<String, String> bodyData = {"email": email.trim().toLowerCase(), "password": password.trim(), "deviceId": deviceId.trim(), "fcmToken": fcmToken.trim()};
    ...
    try {
      ...
      if (profileImage.isNotEmpty) {
        ...
        if (await file.exists()) {
          ...
          formData.files.add(MapEntry("profile", await MultipartFile.fromFile(file.path, filename: fileName, contentType: MediaType.parse(mimeType ?? "application/octet-stream"))));
        }
      }
  ...
  Future<bool> signUp({
    ...
    try {
      FormData formBodyData = FormData.fromMap({"firstName": firstName, "lastName": lastName, "email": email, "role": role, "mobileNumber": mobileNumber, "password": password});
      ...
      for (var element in drivingPhoto) {
        ...
        if (await file.exists()) {
          String fileName = file.path.split('/').last;
          var mimeType = lookupMimeType(file.path);
          formBodyData.files.add(MapEntry("image", await MultipartFile.fromFile(file.path, filename: fileName, contentType: MediaType.parse(mimeType ?? "application/octet-stream"))));
        }
      }
